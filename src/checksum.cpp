...
/*
    ...
    while (pbVar3 = *(byte **)(puVar2 + 4), pbVar3 < buffer + ((len & 0xffff) - 1)) {

    uVar1 = *(short *)puVar2 << 1;

    *(ushort *)puVar2 = uVar1;

      if (0xff < uVar1) {

      *(ushort *)puVar2 = uVar1 & 0xff ^ seed;

      }

      *(byte **)(puVar2 + 8) = pbVar3;

      *(ushort *)puVar2 = *(ushort *)puVar2 ^ (ushort)*pbVar3;

      (byte **)(puVar2 + 4) = pbVar3 + 1;

    }

  }

  return (char)(undefined2 *)puVar2;
